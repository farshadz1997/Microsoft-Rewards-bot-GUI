...
class UserInterface(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.display_errors_checkbox = QtWidgets.QCheckBox(self.global_options_groupbox)
        self.display_errors_checkbox.setObjectName("display_errors_checkbox")
        self.verticalLayout.addWidget(self.display_errors_checkbox)
        ...
        self.current_account.setGeometry(QtCore.QRect(340, 30, 111, 20))
    ...
    def run(self):
        ...
        self.accounts = self.accounts_lineedit.text().split(",")
    ...
    def open_accounts(self):
        ...
        if fileName:
            self.accounts = self.validate_accounts(fileName)
                    
    def validate_accounts(self, path=Path(__file__).parent/"accounts.json"):
        try:
            ...
            self.send_info("Info", "accounts file not found", "accounts file not found,"\
                f"a new one has been created at '{str(accounts_path)}'. Edit it then open it.")
        else:
            for account in accounts:
                if "username" and "password" in account.keys():
                    continue
                else:
                    self.send_error("Error", "Accounts need to have 'username' and 'password'",
    ...
    def get_config(self):
        ...
        try:
            config = json.load(open(f"{Path(__file__).parent}/config.json", "r"))
            ...
            if Path(config["accountsPath"]).is_file():
                self.accounts = self.validate_accounts(config["accountsPath"])
            else:
    ...
    def set_config(self):
        ...
        self.display_errors_checkbox.setChecked(self.config["globalOptions"]["displayErrors"])
        ...
        self.api_key_lineedit.setText(self.config["telegram"]["apiKey"])
    ...
    def save_config(self):
        ...
        config = {
            ...
            "globalOptions": {
                ...
                "displayErrors": self.display_errors_checkbox.isChecked(),
            },
            "farmOptions": {
            ...
            "telegram": {
                "apiKey": self.api_key_lineedit.text(),
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.display_errors_checkbox.setText(_translate("MainWindow", "Display errors"))
